...
- Sub-grouping: when an area grows large, split its files by role for readability — `Application/Loans/{Commands,Queries}`, `Infrastructure/ReadModel/{Projection,Queries}`, `Api/Endpoints/Contracts`. These folders group files **without deepening the namespace** (the type keeps its area namespace), so reorganizing is pure movement with no reference churn. The exception is a distinct published domain concept such as `Domain/Loans/Events`, which keeps its own matching namespace.
